...
1. What is described as the highest-leverage cost optimization?
   * A) Shorter system prompts
   * B) Routing each task to the cheapest model that can do it well (model tiering)
   * C) Turning off streaming
   * D) Using only one provider
   <details><summary>Answer</summary>B) Model tiering - top-tier models can cost 10 to 50 times a small model, so matching task to tier is the biggest lever.</details>

2. Why is a large, unfiltered tool result especially expensive in an agent loop?
   * A) It is generated by the model
   * B) It cannot be cached
   * C) It slows the database
   * D) It is re-sent on every subsequent iteration, so you pay for it repeatedly
   <details><summary>Answer</summary>D) It is re-sent on every subsequent iteration - context is paid for on each loop call, so distill and cap it.</details>

3. You need to extract every column for a document. Which approach reduces both cost and latency?
   * A) One model call per cell
   * B) Re-sending the full transcript on each call
   * C) Extract all columns in one batched call instead of one call per cell
   * D) Use the top-tier model with reasoning on
   <details><summary>Answer</summary>C) One batched call instead of one per cell - fewer iterations means fewer calls, less cost, and less latency.</details>

4. Bulk extraction across documents is independent per document. What cuts wall-clock latency (though not token cost)?
   * A) Running them serially
   * B) Parallelizing the independent work, bounded by rate limits
   * C) Adding reasoning to each call
   * D) A larger context window
   <details><summary>Answer</summary>B) Parallelizing independent work - it does not reduce tokens but slashes the wall-clock time users feel.</details>

5. Does streaming reduce real latency?
   * A) Yes, it makes the model generate faster
   * B) Yes, it reduces token costs
   * C) No - it does not reduce real latency, but it transforms perceived latency by showing progress
   * D) No, and it has no effect on the user experience
   <details><summary>Answer</summary>C) No, it does not reduce real latency - it masks it by turning dead time into visible progress.</details>

### Coding Challenge

**Route a task to its model tier**

Write `route(task)` that maps a task type to its model tier and whether reasoning is on, using a sensible default for unknown tasks. Interactive chat goes top tier with reasoning on; bulk work goes mid tier with reasoning off; trivial work goes low tier.

<details>
<summary>Python Solution</summary>

```python
TIERS = {"top": "gpt-4o", "mid": "gpt-4o-mini", "low": "gpt-3.5"}
TASK_TIER = {
    "chat": ("top", True),
    "bulk_extract": ("mid", False),
    "title": ("low", False),
}


def route(task):
    """Map a task to its model tier and whether reasoning is on."""
    tier, reasoning = TASK_TIER.get(task, ("top", True))   # safe default
    return {"model": TIERS[tier], "reasoning": reasoning}


print(route("chat"))          # {'model': 'gpt-4o', 'reasoning': True}
print(route("bulk_extract"))  # {'model': 'gpt-4o-mini', 'reasoning': False}
print(route("title"))         # {'model': 'gpt-3.5', 'reasoning': False}
```
...
```javascript
const TIERS = { top: "gpt-4o", mid: "gpt-4o-mini", low: "gpt-3.5" };
const TASK_TIER = {
  chat: ["top", true],
  bulk_extract: ["mid", false],
  title: ["low", false],
...
function route(task) {
  const [tier, reasoning] = TASK_TIER[task] || ["top", true]; // safe default
  return { model: TIERS[tier], reasoning };
}

console.log(route("chat"));         // { model: 'gpt-4o', reasoning: true }
console.log(route("bulk_extract")); // { model: 'gpt-4o-mini', reasoning: false }
console.log(route("title"));        // { model: 'gpt-3.5', reasoning: false }
```
