...
6. What range does the chapter give as a common hard iteration cap?
   - A) 1-3 turns
   - B) 5-15 turns
   - C) 50-100 turns
   - D) No cap; rely on the model to stop
   <details><summary>Answer</summary>B) 5-15 turns - a maximum number of loop turns, commonly in that range, and the cap itself is described as non-negotiable.</details>

7. When the iteration cap is hit, what should the loop do?
   - A) Throw an error so the caller knows something went wrong
   - B) Silently retry from the beginning
   - C) Return whatever the model has produced so far, or a graceful "I wasn't able to complete this"
   - D) Escalate to a more capable model automatically
   <details><summary>Answer</summary>C) Return whatever the model has produced so far, or a graceful message - the cap converts an infinite loop into a bounded, eventually-terminating process rather than an error path.</details>

8. Where should persistence happen relative to the loop?
   - A) Inside the loop, saving after every model call
   - B) At the edges: load history before the loop, save the outcome after it
   - C) Only in the provider SDK
   - D) Nowhere; the loop should hold state in memory across turns
   <details><summary>Answer</summary>B) At the edges - keeping the loop stateless makes it a pure function of its inputs, which makes it testable, retryable, and easy to reason about.</details>

9. You want a UI that shows tool activity as it happens, plus a durable record you can debug later. What should the loop produce?
   - A) Only the final text, with the UI inferring what happened
   - B) An ordered, typed event timeline emitted as the loop runs
   - C) A full transcript of the raw provider stream, unparsed
   - D) A summary written by a second model afterwards
   <details><summary>Answer</summary>B) An ordered, typed event timeline emitted as it runs - it drives a rich UI, is the durable record of the turn, and is the raw material for observability. Don't try to reconstruct what happened from the final text.</details>

10. A task is long and the user wants to see and edit the steps before they run. Which loop variant fits, and what is the cost?
    - A) Single-loop ReAct; cost is reduced visibility
    - B) Planner-executor; cost is added complexity and latency
    - C) Multi-agent; cost is nothing measurable
    - D) Reflection loop; cost is fewer model calls
    <details><summary>Answer</summary>B) Planner-executor; cost is added complexity and latency - a first call produces an explicit plan, which gives you structure and steerability in exchange for that complexity.</details>
...
1. The core of every agent fits in about eight lines of pseudocode. So why is the loop the part that first implementations reliably get wrong?
   <details><summary>Show answer</summary>Because the eight lines hide a bookkeeping contract with the provider. When the model asks for a tool, you have to append both the model's tool-request turn and the matching tool-result turn before calling again - the model needs to see "I asked for X, and X returned Y" to continue coherently. Most provider APIs also insist that every tool call in a turn gets a corresponding result, with matching ids. Skip a result, mismatch an id, or append only one of the two turns, and the next call breaks in a way that has nothing to do with your logic. The shape of the loop is trivial; honouring the pairing rules is where the bugs live.</details>

2. The most important line in an agent loop is arguably the one that contributes nothing to the answer: the iteration cap. Why does it earn that status?
   <details><summary>Show answer</summary>Because without it, a confused model can loop forever: call a tool, dislike the result, call again, ad infinitum - and every iteration costs real money and real tokens. The cap is what converts "potentially infinite loop" into "bounded, eventually-terminating process," which is the difference between a system you can put in front of users and one you can't. Note the recommended failure behaviour too: when the cap trips you return what you have, or a graceful apology, rather than erroring. The cap isn't there to make the agent smarter; it's there to make the worst case survivable.</details>

3. More specialised agents coordinating on a task sounds obviously better than one agent doing everything. Why does the chapter tell you to resist it?
   <details><summary>Show answer</summary>Because each extra agent multiplies cost, latency, and failure modes, and the coordination overhead between them is real work that nobody budgets for. The uncomfortable observation in the chapter is that most products which think they need multi-agent actually need better tools and better context in a single loop - the symptom looked like "one agent can't handle this" when the cause was a vague tool description or a context window stuffed with the wrong things. Multi-agent earns its keep when sub-tasks are genuinely independent and benefit from isolation. Start with the single ReAct-style loop and make it prove insufficient first.</details>

4. Your loop runs beautifully and returns a perfect final answer. Why is that, on its own, a design failure?
   <details><summary>Show answer</summary>Because everything interesting happened on the way to that answer, and you threw it away. A loop that returns only final text leaves you with no way to show tool activity in the UI as it happens, no durable record of what the turn actually did, and nothing to debug when a later turn goes wrong. The fix is to emit an explicit, typed event for every meaningful action in order - model reasoned, called tool X, tool X returned, model wrote text - as it occurs. Trying to reconstruct that timeline from the final text afterwards is guesswork, and it's guesswork about exactly the moments you most need to be sure about.</details>
