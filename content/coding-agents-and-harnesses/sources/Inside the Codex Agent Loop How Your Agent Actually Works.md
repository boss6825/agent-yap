# Inside the Codex Agent Loop: How Your Agent Actually Works | Codex Knowledge Base
...
[architecture](/tags/#tag-architecture)[agent-loop](/tags/#tag-agent-loop)[context-management](/tags/#tag-context-management)[internals](/tags/#tag-internals)[performance](/tags/#tag-performance)[advanced](/tags/#tag-advanced)[codex-cli](/tags/#tag-codex-cli)
...
_Based on Michael Bolin’s “Unrolling the Codex Agent Loop” series (January 2026). Source: https://openai.com/index/unrolling-the-codex-agent-loop/_
...
**Why this matters:** The model processes _every input token_ before it can produce _any output token_. A verbose AGENTS.md file, a large file read, or a lengthy tool call result all add latency to every subsequent inference call in the session.
...
> _“Because the agent can execute tool calls that modify the local environment, its ‘output’ is not limited to the assistant message. In many cases, the primary output of a software agent is the code it writes or edits on your machine.”_
...
One session, one task. Use subagents for multi-phase work

Tool calls accumulate fast

Prefer `apply_patch` over shell cat+edit round-trips

Caching rewards stable prompts

Lock AGENTS.md before starting a long session

Parallelism needs App Server

For agentic pods, don’t fight the CLI — use the protocol
...
Branch before risky phases rather than rolling back
...
_Published: 2026-03-28 · Daniel Vaughan · Primary source: Michael Bolin, OpenAI (January 2026)_