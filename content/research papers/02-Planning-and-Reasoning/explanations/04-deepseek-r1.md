...
The team started from a base model and applied [reinforcement learning](./glossary.md#reinforcement-learning), the trial-and-error training you met in folder 01. But here is the twist that makes it work for reasoning. For problems like math and coding, you can check an answer **automatically and for certain**: a math answer is either correct or not, code either passes the tests or not. These are [verifiable rewards](./glossary.md#verifiable-rewards), and they are powerful because they are cheap and impossible to fake.

This sidesteps the whole machinery of folder 01's alignment. There is no separate learned [reward model](./glossary.md#reward-and-reward-signal) that might be fooled. The reward comes straight from checking whether the answer is right.
...
The first model, called R1-Zero, was trained with **pure** reinforcement learning, with no [supervised fine-tuning](./glossary.md#supervised-fine-tuning-sft) warm-up at all, no human-written reasoning examples first. It was simply rewarded for correct answers, over and over.

What happened next is the heart of the paper. With nothing but this reward, the model **taught itself to reason**. It began, entirely on its own, to write longer [chains of thought](./glossary.md#chain-of-thought), to double-check its own work, to try an approach and then reconsider it. Nobody programmed these behaviors in. They [emerged](./glossary.md#emergent-ability) because they led to more correct answers, and correct answers were rewarded.
...
To fix this, the team built the full DeepSeek-R1 with a multi-stage recipe. The key addition is a small amount of [cold-start data](./glossary.md#cold-start-data): a curated set of clean, well-formatted reasoning examples used to gently warm up the model **before** the reinforcement learning begins. This gives the model good habits of presentation first, and then RL sharpens its reasoning.
...
Next: [Chapter 5, Recursive Language Models](./05-recursive-language-models.md), where a model learns to handle inputs far larger than its own memory.
