...
A [benchmark](../../01-Foundational-Modelling/explanations/glossary.md#benchmark) is just a shared test that everyone runs their model on, so that scores can be compared fairly. That sounds simple, but good benchmarks are genuinely difficult to build, and the three papers here each solve a different piece of the puzzle. Together they cover the three things you most want to know about a model: how **broad** its abilities are, how **useful** it is on real work, and which model people actually **prefer** to talk to.

## How to use these explanations

Read the chapters in order; they are arranged from the most traditional style of testing to the most modern. Bold linked terms like [ground truth](./glossary.md#ground-truth) jump to the [glossary](./glossary.md), where every key idea is explained from scratch. Foundational terms that belong to the earlier folders (such as what a [benchmark](../../01-Foundational-Modelling/explanations/glossary.md#benchmark) is at heart, or what an [Elo rating](../../01-Foundational-Modelling/explanations/glossary.md#elo-rating) is) are linked across to those folders rather than repeated here.
...
|-------|------|------------------|---------|
| BIG-Bench (Beyond the Imitation Game) | 2022 | Breadth: a huge, diverse suite of 204 tasks | [Chapter 1](./01-big-bench.md) |
| SWE-bench | 2023 | Real-world usefulness: fixing actual software bugs | [Chapter 2](./02-swe-bench.md) |
| Chatbot Arena | 2024 | Human preference: which model people actually like | [Chapter 3](./03-chatbot-arena.md) |

## Chapter map

1. [BIG-Bench: testing the full breadth of what a model can do](./01-big-bench.md)
2. [SWE-bench: can a model fix a real bug in real code?](./02-swe-bench.md)
3. [Chatbot Arena: letting real people pick the winner](./03-chatbot-arena.md)
4. [Glossary: every key term, explained from zero](./glossary.md)
...
Ready? Start with [Chapter 1: BIG-Bench](./01-big-bench.md).
